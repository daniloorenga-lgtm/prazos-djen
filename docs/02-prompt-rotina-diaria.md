...
4. Execute `python src/rodar.py --etapa contar-e-lancar`. Este passo conta os prazos, verifica duplicidade e cria os cartões (fatal + lembrete). Leia a saída: qualquer linha `ERRO` ou `NAO_LANCADO` deve constar no e-mail.
